...
* datasource/redis/port/profile 등은 조회 전용이며 실제 변경은 재시작이 필요합니다.`);
  ...
  async function loadDebugConfig() {
    try {
      ...
      setVal('ollamaBaseUrlInput', data.ollamaBaseUrl || '');
...
maxUploadFileCount=${data.ragMaxUploadFileCount}`, 'success');
  ...
  function setOllamaConnectionSummary(info) {
    if (!info) { setText('ollamaConnectionSummary', '연결 정보 미조회'); return; }
    setText('ollamaConnectionSummary', `baseUrl=${info.baseUrl || '-'}
status=${info.status || '-'}
reachable=${info.reachable}
running=${info.runningCount ?? 0}
installed=${info.installedCount ?? 0}
message=${info.message || '-'}`);
  }
  async function loadOllamaConnection() {
    try {
      const info = await fetchJson('/debug/api/ollama/connection');
      setVal('ollamaBaseUrlInput', info.baseUrl || '');
      setOllamaConnectionSummary(info);
    } catch (e) {
      setOllamaConnectionSummary({ baseUrl: val('ollamaBaseUrlInput'), status: 'ERROR', reachable: false, runningCount: 0, installedCount: 0, message: e.message });
    }
  }
  async function saveOllamaConnection() {
    try {
      const info = await fetchJson('/debug/api/ollama/connection', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ baseUrl: val('ollamaBaseUrlInput') }) });
      setVal('ollamaBaseUrlInput', info.baseUrl || '');
      setOllamaConnectionSummary(info);
      await loadDebugConfig();
  ...
  async function resetOllamaConnection() {
    try {
      const info = await fetchJson('/debug/api/ollama/connection/reset', { method: 'POST' });
      setVal('ollamaBaseUrlInput', info.baseUrl || '');
      setOllamaConnectionSummary(info);
      await loadDebugConfig();
      await loadOllamaModels();
...
trace=${data.result.traceId || '-'}`, data.result.failCount > 0 ? 'running' : 'success');
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    updateActionButtons();
    await loadDebugConfig(); await loadOllamaConnection(); await loadOllamaModelConfig(); await loadRagStatus(); await loadDbInfo(); await loadSources(); await loadLatestRagTraces();
  });
})();
